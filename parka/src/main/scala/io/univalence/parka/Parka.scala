...
object Parka {
  ...
  def describe(row: Row)(keys:Set[String]):Map[String,Describe] = {
    ...
    fields.filterNot(keys).map(name => {
  ...
  def outer(row: Row, side: Side)(keys:Set[String]):Outer = {
    Outer(
      ...
      byColumn = describe(row)(keys).mapValues(d =>
        side match {
  ...
  def inner(left:Row, right: Row)(keys:Set[String]):Inner = {
    ...
    val byNames: Map[String, Delta] = schema.fieldNames.filterNot(keys).map(name => {
  ...
  private val emptyInner: Inner = Monoid.gen[Inner].empty
  private val emptyOuter: Outer = Monoid.gen[Outer].empty
  private val emptyDescribe: Describe = implicitly[Monoid[Describe]].empty

  private val emptyResult:ParkaResult = Monoid.gen[ParkaResult].empty
...
val keys = keyNames.toSet
    ...
    ParkaAnalysis(datasetInfo = Both(leftDs, rightDs).map(datasetInfo),
      result = res)
  }
