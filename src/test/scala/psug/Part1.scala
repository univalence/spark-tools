...
trait Applicative[M[_]] {
  ...
  def apply2[A, B, C](m1: M[A], m2: M[B])(f: (A, B) => C): M[C] = ap(m2)(map(m1)(f.curried))
...
object IronSuit {
  ...
  implicit val applicativeAltInst: ApplicativeAlternative[IronSuit] = new ApplicativeAlternative[IronSuit] {
    override def point[A](a: A): IronSuit[A] = IronSuit(a)

    override def zip[A, B](m1: IronSuit[A], m2: IronSuit[B]): IronSuit[(A, B)] = IronSuit((m1.value, m2.value))

    override def map[A, B](m1: IronSuit[A])(f: (A) => B): IronSuit[B] = IronSuit(f(m1.value))
  }
}
