...
trait Applicative[M[_]] {
  ...
  def apply[A,B,C](m1:M[A],m2:M[B])(f:(A,B) => C):M[C] = ap(m2)(map(m1)(f.curried))
...
val kY = Witness.`'y`
type Res = Option[FieldType[kName.T, String]] :: Option[FieldType[kY.T ,Int]] :: HNil
implicit val optionAppInstance:Applicative[Option] = new Applicative[Option] {
  override def point[A](a: A): Option[A] = Option(a)

  override def ap[A, B](m1: Option[A])(m2: Option[(A) => B]): Option[B] = m2.flatMap(f => m1.map(f))
}
...
def zip[A[_],X,Y](x:A[X])(y:A[Y])(implicit app:Applicative[A])=app.apply(x,y)((x,y) => (x,y))
