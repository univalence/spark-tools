...
import org.apache.spark.sql.catalyst.expressions.{ Alias, Attribute, AttributeReference, CreateArray, CreateStruct, Expression, GenericRowWithSchema, GetStructField, Literal, NamedExpression, ScalaUDF }
  ...
  private def annotationsFusion[T](s: Seq[T]): Seq[T] = {
    if (s.isEmpty) {
      ...
      case _: GenericRowWithSchema => s.asInstanceOf[Seq[GenericRowWithSchema]].groupBy(x => x.toSeq.updated(x.fieldIndex("count"), 0)).values.map(x => {
        new GenericRowWithSchema(x.head.toSeq.updated(x.head.fieldIndex("count"), x.map(_.getAs[Long]("count")).sum).toArray, x.head.schema)
      }).toSeq.asInstanceOf[Seq[T]]
      case _ => ???
    }
    ...
    private val annotationsDt =
      ArrayType(
        StructType(Seq(
          ...
          StructField(name = "count", dataType = LongType, nullable = false))), containsNull = false)
    ...
    private def recursiveNewPlan(logicalPlan: LogicalPlan, sparkSession: SparkSession): (LogicalPlan, Seq[Attribute]) = {
      ...
      val res = logicalPlan match {
        ...
        case agg @ Aggregate(groupingExpressions, aggregateExpressions, child) =>
          ...
          val annotationCol: NamedExpression = Alias(mergeAnnotationUDF(Column(CreateArray(attributes :+ currentLevel))).expr, "annotations")()
