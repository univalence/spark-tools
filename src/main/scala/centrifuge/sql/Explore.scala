...
case class Address(personName: String,
                   street: String,
                   postcode: String,
                   city: String)
...
object Explore {
  ...
  def add_annotations(seq1: Seq[Annotation],
                      seq2: Seq[Annotation]): Seq[Annotation] = seq1 ++ seq2
  ...
  def reduce_annotations(
      seq: Seq[GenericRowWithSchema]): Seq[GenericRowWithSchema] = {
    seq
      .groupBy(x ⇒
        (x.getAs[Any]("msg"), x.getAs[Any]("origin"), x.getAs[Any]("isError")))
      .map(t ⇒ {
        val toArray = t._2.head.toSeq.toArray
        toArray.update(t._2.head.fieldIndex("count"),
                       t._2.map(_.getAs[Long]("count")).sum)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sql1 = sQLContext.sql(
      "select to_age(age) as age, non_empty_string(name) as name from person")
    //sql1.show(false)
    ...
    val collect: Seq[Option[Expression]] = plan.expressions.collect({
      case Alias(child, colname) ⇒ {
        child match {
          case s @ ScalaUDF(f, dt, children, inputTypes, Some(name))
              if name == "non_empty_string" ⇒
            Some(s)
          case _ ⇒ None
        }
      }
