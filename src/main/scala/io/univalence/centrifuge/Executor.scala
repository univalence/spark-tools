...
object RetryDs {
  def retryDs[A, C, B](in: Dataset[A])(run: A ⇒ Try[C])(integrate: (A, Try[C]) ⇒ B)(nbGlobalAttemptMax: Int, circuitBreakerMaxFailure: Int = 10)(implicit
    encoderA: Encoder[A],
                                                                                                                                                 encoderB: Encoder[B],
  ...
  def retryDsWithTask[A, C, B](in: Dataset[A])(run: A ⇒ Task[C])(integrate: (A, Try[C]) ⇒ B)(nbGlobalAttemptMax: Int, circuitBreakerMaxFailure: Option[Int] = Option(10))(implicit
    encoderA: Encoder[A],
                                                                                                                                                                          encoderB: Encoder[B],
                                                                                                                                                                          encoderI: Encoder[(Option[A], LocalExecutionStatus, B)]
      ...
      if (mAndEs._2.nbFailure == 0 || attemptRemaining <= 0)
        ...
        Task {
          ...
          val newDs: Dataset[(Option[A], LocalExecutionStatus, B)] = ds.mapPartitions(iterator ⇒ {
            ...
            iterator.map({
              case (Some(a), _, _) ⇒ aToM(a, x ⇒ circuitBreaker.fold(x)(breaker ⇒ breaker.protect(x)))
              case x               ⇒ x
            })
          })
          newDs.persist()
