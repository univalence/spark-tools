    ...
    if(!circuitClosed) {
      ...
      t match {
        ...
        case Failure(e) =>
          ...
          ExecutionResult(None,Some(ExecutionInfo(1,skipped = false,Some(e.getMessage))))
      }
    }
  }
}

case class Executor protected(config: ExecutorConfig) {
  @transient private lazy val myThreadLocal: ThreadLocal[ThreadLocalExecutor] =
    ThreadLocal.withInitial(() => new ThreadLocalExecutor(config = config))
...
case class ExecutorConfig(name: String,
                          attempt: Int,
...
case class ExecutorBuildRep(name: Option[String] = None,
                            ...
                            maxNbCallPerSeconds:Option[Long] = None) extends ExecutorBuilder {
  ...
  override def getOrCreate(): Executor = {
    ...
    Executor(ExecutorConfig(name = name.get,attempt.filter(_ > 1).getOrElse(1), breakAfterNFailure = breakAfterNbFailure))
  }

  override def limitRate(nbCallPerSeconds: Long): ExecutorBuilder = copy(maxNbCallPerSeconds = nbCallPerSeconds)
}

trait ExecutorBuilder {
  def name(name: String = UUID.randomUUID().toString): ExecutorBuilder

  def limitRate(nbCallPerSeconds:Long):ExecutorBuilder
...
object PrgSpark {
  ...
  def retryDs[A <: Product : TypeTag, B <: Product : TypeTag, CallRes <: Product : TypeTag]
  (in: Dataset[A])
  (run: A => Try[CallRes])
  (integrate: (A, Try[CallRes]) => B)
  (nbAttemptStageMax: Int,
   ...
   recoverOutput: A => Option[CallRes] = (a: A) => None
    ...
    @tailrec
    def innerLoop(ds: Dataset[(A, ExecutionResult[CallRes], B)],
                  remainingAttempts: Int,
                  oldDs: Option[Dataset[(A, ExecutionResult[CallRes], B)]]): Dataset[B] = {
