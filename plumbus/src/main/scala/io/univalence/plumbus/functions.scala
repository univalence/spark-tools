...
object functions {
  ...
  implicit class RicherTypedColumnSeq[T, A](typedColumn: TypedColumn[T, Seq[A]]) {
    ...
    def map[B](f: A => B)(
      implicit cleanFromRow: CleanFromRow[A],
      typeTagB: TypeTag[B],
      typeTag: TypeTag[A],
      encoder: Encoder[Seq[B]]
    ): TypedColumn[Any, Seq[B]] = {
      val f0 = udf[Seq[B], Seq[A]](serializeAndClean(s => s.map(f)))
      f0(typedColumn).as[Seq[B]]
    }

    def filter(p: A => Boolean)(
      implicit encoder: Encoder[Seq[A]],
      typeTag: TypeTag[A],
      cleanFromRow: CleanFromRow[A]
    ): TypedColumn[Any, Seq[A]] = {
      val p0 = udf[Seq[A], Seq[A]](serializeAndClean(s => s.filter(p)))
      p0(typedColumn).as[Seq[A]]
    }

    def flatMap[B](f: A => Seq[B])(
      implicit encoder: Encoder[Seq[B]],
      typeTagA: TypeTag[A],
      typeTag: TypeTag[B],
      cleanFromRow: CleanFromRow[A]
    ): TypedColumn[Any, Seq[B]] = {
  ...
  def differentLines(left: DataFrame, right: DataFrame): Dataset[Row] = {
    ...
    left
      .withColumn("side", lit("left"))
      .union(right.withColumn("side", lit("right")))
      .groupBy(left.columns.map(left.apply): _*)
      .agg(
