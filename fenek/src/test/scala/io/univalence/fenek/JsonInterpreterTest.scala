...
class JsonInterpreterTest extends FunSuite {
  ...
  test("75.0.as[BigDecimal]") {

    struct("a" <<- 75.0.as[BigDecimal]).setExpected("a" -> BigDecimal(75)).check()
  }
  ...
  test("cast double to BigDecimal") {
    struct("a" <<- "2.5".as[BigDecimal], "b" <<- "2".as[BigDecimal])
      .setExpected("a" -> BigDecimal(2.5), "b" -> BigDecimal(2))
      .check()
  }
